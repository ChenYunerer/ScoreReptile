...
func startQuPu123ReptileTask() {
	//创建最顶级任务
	nowTime := time.Now()
	taskName := fmt.Sprintf("%d-%d-%d 中国曲谱网爬虫任务", nowTime.Year(), nowTime.Month(), nowTime.Day())
	taskInfo := model.CreateBasicTaskInfo(taskName, 1)
...
func StartNetEaseMusicReptileTask() {
	//创建最顶级任务
	nowTime := time.Now()
	taskName := fmt.Sprintf("%d-%d-%d 网易云音乐爬虫任务", nowTime.Year(), nowTime.Month(), nowTime.Day())
	taskInfo := model.CreateBasicTaskInfo(taskName, 2)
	if _, err := db.Engine.InsertOne(taskInfo); err != nil {
		log.Println(err)
	}
	childTaskInfoList := make([]model.ReptileTaskInfo, 0)
	...
	//执行第一步操作 抓取各个类目的列表数据
	topListTask := startReptileNetEaseMusicTopList(*taskInfo)
	childTaskInfoList = append(childTaskInfoList, topListTask)
